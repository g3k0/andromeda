    ...
    <div className="space-y-6">
      <header className="space-y-1">
        <h1 className="text-2xl font-bold tracking-tight">
          Reading copy #{token.copyNumber ?? token.tokenId.toString()}
        </h1>
        <p className="text-sm text-white/60">Work #{work.workId.toString()}</p>
      </header>

      <WorkReaderClient
        tokenId={token.tokenId.toString()}
        metadataUrl={toGatewayUrl(work.metadataURI, gatewayBaseUrl)}
        envelopeUrl={
          token.envelopeCid

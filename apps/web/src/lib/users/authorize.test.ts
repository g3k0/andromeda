...
import { buildAuthenticatedUser } from "./testing/build-authenticated-user";
...
describe("user authorize", () => {
  it("allows admins to read and write users", () => {
    const admin = buildAuthenticatedUser(ADMIN, "admin");
  ...
  it("allows self-read but not cross-user reads for readers", () => {
    const reader = buildAuthenticatedUser(READER, "reader");
  ...
  it("allows authors to edit only their own profile", () => {
    const author = buildAuthenticatedUser(AUTHOR, "author");
